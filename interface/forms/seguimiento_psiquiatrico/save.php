...
function prescription($form_id,$user,$pid,$encounter){
        $d_id = sqlQuery("delete from seguimiento_medicamento1 where form_id=?",$form_id);
	$d_id = sqlQuery("delete from seguimiento_medicamento2 where form_id=?",$form_id);
	global $prescription;
	global $prescription1;
        foreach($prescription1 as $k => $value){
		  ...
		  $p_id = sqlInsert("insert into seguimiento_medicamento1 (user,pid,encounter,form_id,medicamento,dosis,size,unit,amount,form,route,`interval`,refills,freq_other) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?)", $arg);
          }
	}
	foreach($prescription as $k => $value){
          if($value['medicamento'] != ''){
                         $arg = array($user,$pid, $encounter,$form_id,$value['medicamento'],$value['dosis'],$value['size'],$value['unit'],$value['amount'],$value['form'],$value['route'],$value['interval'],$value['refills'],$value['freq_other']);
                         $p_id = sqlInsert("insert into seguimiento_medicamento2 (user,pid,encounter,form_id,medicamento,dosis,size,unit,amount,form,route,`interval`,refills,freq_other) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?)", $arg);
          }
        }
