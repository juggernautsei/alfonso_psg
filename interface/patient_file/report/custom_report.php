...
$SEND_EMAIL = empty($_POST['email']) ? 0 : intval($_POST['email']);
$EMAIL_TO = empty($_POST['email_address']) ? '' : $_POST['email_address'];
...
function getInvalidEmailAddress($emails) {
    $invalidEmails = [];
...
<div class="container">
    <div id="report_custom w-100">  <!-- large outer DIV -->
        ...
        if ($printable) {
            ...
            echo "<div style='widht:100%;'><table width='100%'><tbody><tr><td style='width:33%;'></td><td style='text-align:center;width:33%'>";
            if (file_exists($practice_logo)) {
                $logo_path = $GLOBALS['OE_SITE_WEBROOT'] . "/images/" . basename($practice_logo);
                echo "<img style='max-width:250px;height:auto;' src='$logo_path'>"; // keep size within reason
                ...
                <div class="row">
                    <div class="col-md">
                        <input type="text" class="form-control"
                               onkeyup="clear_last_visit();remove_mark_all();find_all();" name="search_element"
                               id="search_element"/>
                    </div>
                    <div class="col-md">
                        <div class="btn-group">
                            <button type="button" class="btn btn-primary btn-search"
                                    onClick="clear_last_visit();remove_mark_all();find_all();"><?php echo xlt('Find'); ?></button>
                            <button type="button" class="btn btn-primary"
                                    onClick="next_prev('prev');"><?php echo xlt('Prev'); ?></button>
                            <button type="button" class="btn btn-primary"
                                    onClick="next_prev('next');"><?php echo xlt('Next'); ?></button>
                        </div>
                    </div>
                    <div class="col-md">
                    ...
                    if (($auth_notes_a || $auth_notes || $auth_coding_a || $auth_coding || $auth_med || $auth_relaxed)) {
                        ...
                        <div name="search_div" id="search_div_<?php echo attr($form_id) ?>_<?php echo attr($res[1]) ?>"
                            ...
                            if ($esign->isLogViewable("report")) {
                                $bres = sqlStatement(
                                    "SELECT tid, uid, is_lock, fname, lname, info, npi, federaldrugid, dm ,bn, lic FROM esign_signatures as e LEFT JOIN users as u " .
                                    "ON e.uid = u.id WHERE e.tid = ? " .
                                    "ORDER BY e.datetime desc",
                                    array($formId)
                                );
                                while ($brow = sqlFetchArray($bres)) {
                                    $sign_path = $GLOBALS['OE_SITE_WEBROOT'] . "/images/sign_" . $brow['uid'] . ".JPG";
                                    echo "</br>";
                                    echo "<table class='table'>
						    <tr><td style='border-top:none'>
						    " . $brow['fname'] . " " . $brow['lname'] . " <br> NPI : " . $brow['npi'] . "
						    <br> LIC : " . $brow['lic'] . "<br> DEA : " . $brow['federaldrugid'] . "
						   <br> DM : " . $brow['dm'] . "
							<br> BN: " . $brow['bn'] . "<br> " . $brow['info'] . "
						</td><td class='text-right' style='border-top:none'>
                        <img style='max-width:250px;height:auto;' src='$sign_path'>
</td>
							</tr></table>";
                                    #$esign->renderLog();
                                }
...
if ($PDF_OUTPUT) {
    ...
    if ($PDF_OUTPUT == 1) {
        if ($SEND_EMAIL == 1 && $EMAIL_TO != '') {
            ...
            $ptdata = getPatientData($pid);

            if ($GLOBALS['EMAIL_METHOD'] === 'SMTP') {
                if (empty($GLOBALS['SMTP_HOST']) && empty($GLOBALS['SMTP_SECURE']) && empty($GLOBALS['SMTP_USER']) && empty($GLOBALS['SMTP_PASS']) && empty($GLOBALS['SMTP_PORT'])) {

                    echo "Mailer Error: Please set required fields for SMTP settings. Please configure SMTP settings from Globel -> Notifications....";
                    echo '<script>alert("Please set required fields for SMTP settings. Please configure SMTP settings from Globel -> Notifications....")</script>';
                } else {

                    //To email address validation
                    $addr = explode(',', $EMAIL_TO);
                    $invalidEmails = getInvalidEmailAddress($addr);
                    if (count($invalidEmails) > 0)
                    {
                        $validationErrorMsg = "Invalid email address: ".implode(', ', $invalidEmails);
                        echo "<div id='backLink'>
                                            <a href='patient_report.php' onclick='top.restoreSession()'>
                                    <span class='title'>";
                        ...
                        echo '<script>alert("'.$validationErrorMsg.'....")</script>';
                    }else{
                        //Mail Configuration
                        $mail = new MyMailer();
                        $mail->isSMTP();
                        ...
                        $mail->From = 'no-reply@primesupport.com';
                        $mail->FromName = 'Prime Support Group';
                        ...
                        $mail->addAttachment($tmp_file, $ptdata['lname'] . '_report');
                        if (!$mail->Send()) {
                ...
                echo "</span>
                                        ...
                                        <br />
                                        <br />";
                echo "Mailer Error: SMTP Configration is not set. Please configure SMTP settings from Globel -> Notifications....";
                echo '<script>alert("SMTP Configration is not set. Please configure SMTP settings from Globel -> Notifications....")</script>';
            }
