...
if (isset($_GET['iSortCol_0'])) {
    for ($i = 0; $i < intval($_GET['iSortingCols']); ++$i) {
	    ...
	    } else if ($aColumns[$iSortCol] == 'policy_number') {
	        $orderby .= "`" . escape_sql_column_name($aColumns[$iSortCol], array('insurance_data')) . "` $sSortDir";
	    }else{
...
if (isset($_GET['sSearch']) && $_GET['sSearch'] !== "") {
	...
	} elseif($colname == 'policy_number') {
		if ($searchMethodInPatientList) { // exact search
			$where .= "`" . escape_sql_column_name($colname, array('insurance_data')) . "` LIKE ? ";
			array_push($srch_bind, $sSearch);
		} else {
			$where .= " `" . escape_sql_column_name($colname, array('insurance_data')) . "` LIKE ?"; // any search
			array_push($srch_bind, ('%' . $sSearch . '%'));
		}
...
$sellist = 'pid';
foreach ($aColumns as $colname) {
    if ($colname == 'pid' || $colname == 'policy_number') {
        continue;
    }
...
$row = sqlQuery("SELECT COUNT(p.id) AS count FROM patient_data p left join insurance_data i on i.pid=p.pid and i.type='primary' WHERE $customWhere", $boundFilter->getBoundValues());
$iTotal = $row['count'];
...
$row = sqlQuery("SELECT COUNT(p.id) AS count FROM patient_data p left join insurance_data i on i.pid=p.pid and i.type='primary'  WHERE $where", $srch_bind);
...
//array_push($aColumns,'policy_number');
//$aColumns = explode(',', $_GET['sColumns']);
$sellist = "p.".implode(", p.", explode(',',$sellist)). ', i.policy_number';
$query = "SELECT $sellist FROM patient_data as p left join insurance_data i on i.pid=p.pid and type='primary' WHERE $where $orderby $limit";
