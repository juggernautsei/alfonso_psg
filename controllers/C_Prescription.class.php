...
//     }
    ...
    function multiprint_header(&$pdf, $p)
    {
        ...
        //print header
        $pdf->ezImage($GLOBALS['oer_config']['prescriptions']['practice_logo'], '', '200', '', 'center', '');
	...
	echo ("<td colspan='2' style='text-align:center'>\n");
        echo ("<img WIDTH='320pt' src='/sites/default/images/practice_logo.png' />");
        echo ("</td>\n");
        echo ("</tr>\n");
        echo ("<tr>\n");
        echo ("<td>\n");
    ...
    {
	    $res = sqlQuery("SELECT concat('LIC. ',state_license_number,' NPI ',npi,'\n') licnpi,concat('DM ',dm,' BN ',bn,'\n') dmbn,concat(street,', ',streetb,', ',city,', ',state,' ',zip) addr FROM users where id ='" .
            add_escape_custom($p->provider->id) . "'");
...
if ($GLOBALS['rx_enable_DEA']) {
	    ...
	    $pdf->ezText('<b>' . $p->provider->get_name_display() . '</b>', 12);
                $pdf->ezText($res['licnpi'], 12);
                $pdf->ezText($res['dmbn'], 12);
if ($GLOBALS['rx_enable_DEA']) {
            if ($this->is_faxing || $GLOBALS['rx_show_DEA']) {
    ...
    function multiprintcss_footer($p)
    {
	    ...
	    echo ('<b><span class="large">' .  $p->provider->get_name_display() . '</span></b>' . '<br />');
	    echo ('<span class="medium">' .  $res['licnpi'] . '</span>' . '<br />');
	    echo ('<span class="medium">' .  $res['dmbn'] . '</span>' . '<br />');
	if ($GLOBALS['rx_enable_DEA']) {
            if ($GLOBALS['rx_show_DEA']) {
                echo ('<span class="">' . xl('DEA') . ' ' . $p->provider->federal_drug_id . '</span><br />');
            } else {
                echo ('<span class="">' . xl('DEA') . ' </span> ________________________<br />' );
            }
        }
	    echo ('<span class="">' .  $res['addr'] . '</span>' . '<br />');
